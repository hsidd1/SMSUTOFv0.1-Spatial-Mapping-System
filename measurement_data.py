...
'''
Program to write measurement data from ToF to .xyz file
Usage: After iputting number of increments when running this program, 
       Run the respective Keil project utilizing I2C to interface with microcontroller and VL53L1X Time of Flight sensor
       This program will then use UART communication to receive measurements and print to console and write to a .xyz file
       Make sure to close any software using the same port for communication, such as RealTerm, before using
       Modify relevant parameters based on C code configuration
'''
...
with open("pointdata.xyz", "w") as f:
    ...
    for i in range(num_increments):
        ...
        if (steps == 512): #reset number of steps after a full rotation is completed and increament x and count
            steps = 0
            x += STEP_INCREMENT
        print(data)
